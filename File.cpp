...
bool File::str_pfx(const char *str, const char *pfx){
  if(pfx == NULL)
    return true;
  if(str == NULL)
    return false;
...
bool File::str_sfx(const char *str, const char *sfx){
  int i, j;
...
void File::errmsg(const char *msg){
...
void File::setfilename(const char *fname){
  delete filename;
  if(fname == NULL)
    filename = NULL;
  else{
