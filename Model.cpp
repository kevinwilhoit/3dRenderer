...
//
int Model::AddTriangle(int v0, int v1, int v2, const Vector2d &t0, const Vector2d &t1, const Vector2d &t2, int matindice){
...
int Model::AddTriangle(int v0, int v1, int v2){
  Vector2d zero(0, 0);
  AddTriangle(v0, v1, v2, zero, zero, zero, 0);
}
 
//
// Insert a triangle and its normal, without texture coordinates, with material indice into the triangle tables
//
int Model::AddTriangle(int v0, int v1, int v2, int matindice){
  Vector2d zero(0, 0);
  AddTriangle(v0, v1, v2, zero, zero, zero, matindice);
}
...
void Model::BuildTeapot(PolySurf *scenet){
  int i, j;
  scene = scenet;
  Clean();

  for(i = 0; i < scene->nverts; i++)
    AddVertex(scene->verts[i]);

  for(i = 0; i < scene->nfaces; i++){
    if(scene->faces[i].nverts == 3){
       if(scene->nuvs > 0){
          AddTriangle( scene->faces[i].faceverts[0].v, scene->faces[i].faceverts[1].v, 
	               scene->faces[i].faceverts[2].v, scene->uvs[scene->faces[i].faceverts[0].u],
                       scene->uvs[scene->faces[i].faceverts[1].u], scene->uvs[scene->faces[i].faceverts[2].u], scene->faces[i].material );
       } else {
          AddTriangle( scene->faces[i].faceverts[0].v, scene->faces[i].faceverts[1].v, 
	               scene->faces[i].faceverts[2].v, scene->faces[i].material);
       }
    }else {
       if(scene->nuvs > 0){
          AddTriangle( scene->faces[i].faceverts[0].v, scene->faces[i].faceverts[1].v, 
          ...
          AddTriangle( scene->faces[i].faceverts[2].v, scene->faces[i].faceverts[3].v, 
                       scene->faces[i].faceverts[0].v, scene->uvs[scene->faces[i].faceverts[2].u], 
                       scene->uvs[scene->faces[i].faceverts[3].u], scene->uvs[scene->faces[i].faceverts[0].u], scene->faces[i].material );
      } else {
          AddTriangle( scene->faces[i].faceverts[0].v, scene->faces[i].faceverts[1].v, 
                       scene->faces[i].faceverts[2].v, scene->faces[i].material);

          AddTriangle( scene->faces[i].faceverts[2].v, scene->faces[i].faceverts[3].v, 
                       scene->faces[i].faceverts[0].v, scene->faces[i].material);
      }
    }
  }
}
...
void Model::BuildCuboid(float width, float height, float depth){
  ...
  for(i = 0; i < 36; i += 3){
    ...
    AddTriangle(v[vlist[i]], v[vlist[i + 2]], v[vlist[i + 1]],
		tlist[3 * j], tlist[3 * j + 2], tlist[3 * j + 1], 0);
  }
}
...
void Model::Draw(int wireframe){
  ...
  for(itri = 0; itri < ntriangles; itri++){
    ...
    scene->materials[matindex[itri]].createTexture(); 
    glBindTexture(GL_TEXTURE_2D, scene->materials[matindex[itri]].textureid);
    ...
    for(int i = 0; i < 3; i++){
      ...
      int shininess;
      for(int j = 0; j < 3; j++){
        ambient_color[j] = AMBIENT_FRACTION * scene->materials[matindex[itri]].a[j];
        diffuse_color[j] = DIFFUSE_FRACTION * scene->materials[matindex[itri]].d[j];
        specular_color[j] = SPECULAR_FRACTION * scene->materials[matindex[itri]].s[j];
        shininess = 60;
      }
      ...
      if (scene->nnorms > 0)
        glNormal3f(scene->norms[ivertex].x, scene->norms[ivertex].y, scene->norms[ivertex].z);
