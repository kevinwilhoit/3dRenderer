...
#define MAXVERTICES   10000000		  // shapes limited to 10000 vertices
...
private:
  ...
  // insert a triangle and its normal, without texture coords
  int AddTriangle(int v0, int v1, int v2, int matindice);
  // insert a triangle and its normal, with texture coords
  int AddTriangle(int v0, int v1, int v2, const Vector2d &t0, const Vector2d &t1, const Vector2d &t2, int matindice);
...
public:
  ...
  // Make a teapot model
  void BuildTeapot(PolySurf *scenet);
